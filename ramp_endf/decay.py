...
class DecayProcess:
    ...
    def from_other(cls, other: "DecayProcess", **kwargs) -> "DecayProcess":
        ...
        for key in ['parent', '_target', 'halflife', 'mode', 'fraction',
                    'fraction_err', 'energy', 'energy_err', 'spectra']:
...
def get_decay_modes(value):
    ...
    """
    return tuple(_RTYP_DECAY_MODES[int(v)] for v in
...
class DecayMode:
    ...
    def target_branching(self, spf_db: Optional[SPFData] = None
                         ) -> Tuple[Tuple[ZAID, float], ...]:
        ...
        for mode in self.modes:
            ...
            if mode == SPF:
                try:
                    counter.update(
                        {target: yield_ for target, yield_, _ in
                         spf_db[iso]})
                except KeyError:
...
class Decay:
    ...
    def __init__(self, ev_or_filename: Evaluation | Path | IO):
        ...
        if not self.nuclide['stable']:
            ...
            for i in range(n_modes):
                ...
                mode = DecayMode(self.zaid, decay_type,
                                 isomeric_state,
                                 energy, branching_ratio)
                self.modes.append(mode)

            discrete_type = {0.0: None,
    ...
    def decay_processes(self, spf_db: Optional[SPFData] = None) -> List[DecayProcess]:
        spf_db = spf_db or {}
        return [DecayProcess(mode.parent,
...
def parse_decay_processes(*file_or_ev: Path | Evaluation | Decay,
                          spf_db=Optional[SPFData]) -> Sequence[DecayProcess]:
    """parse decay processes from decay files"""
    decays = parse_decays(*file_or_ev)
    return list(it.chain.from_iterable(
        decay.decay_processes(spf_db=spf_db) for decay in decays))
