...
import math
import re
from typing import TextIO, Sequence, Generator, Any, Union, Iterable

import numpy as np
from uncertainties import UFloat, ufloat as raw_ufloat

from .units import Second, CMBarnPerSecond
...
def ufloat(mu: float, sigma: float) -> UFloat | float:
    ...
    return raw_ufloat(mu, sigma) if sigma != 0. else mu
...
def parse_section(of: TextIO, MF: int, MT: int, nuclide: int = 0):
    ...
    pat = re.compile('^(' + 66 * '.'
                     + (f')(?:{nuclide:4d})' if nuclide else f')(?:....)')
                     + f'(?:{MF:2d}{MT:3d})',
                     re.MULTILINE)           
...
def parse_list(lines: Sequence[str], num_vector:int, num_total: int):
    ...
    modlogger.debug("Reading a list for %d vectors, total of %d numbers",
                    num_vector, num_total)   
...
def halflife_to_rate(halflife: Second,
                     branching: float = 1.) -> CMBarnPerSecond:
