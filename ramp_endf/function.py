...
class Tabulated1D:
    ...
    >>> [f(xi).item() for xi in np.linspace(0, 10, 5)]
    [4, 4.25, 4.5, 4.75, 5]
    ...
    @staticmethod
    def _partial_sum(x, y, breakpoints, interpolation):
        # Create output array
        partial_sum = np.zeros(len(x) - 1)

        i_low = 0
        for k in range(len(breakpoints)):
            # Determine which x values are within this interpolation range
            i_high = breakpoints[k] - 1

            # Get x values and bounding (x,y) pairs
            x0 = x[i_low:i_high]
            x1 = x[i_low + 1:i_high + 1]
            y0 = y[i_low:i_high]
            y1 = y[i_low + 1:i_high + 1]
    ...
    def histogram(self, xs):
        x = np.union1d(self.x, xs)
        xs_new = np.setdiff1d(xs, self.x)
        x_mid = 0.5 * (x[:-1] + x[1:])
        y = self(x)
        x_edges = x[np.hstack((0, self.breakpoints - 1))]
        breakpoints = self.breakpoints + np.histogram(xs_new, x_edges)[0]
        partial_sums = self._partial_sum(x, y, breakpoints, self.interpolation)
        return np.histogram(x_mid, xs, weights=partial_sums)[0]
    ...
    def integral(self):
        ...
        """
        partial_sum = self._partial_sum(self.x, self.y, self.breakpoints,
                                        self.interpolation)
