...
try:
    from batman.graphs import draw
    from batman.graphs.decay import DecayGraph
    from batman.graphs.filters import subsets_connection_subgraph, blacklist_filter, exclude_spf_filter
except ImportError:
    DecayGraph = None
    
from ramp_endf import parse_decay_processes
from ramp_endf import parse_spontaneous_fission
from ramp_endf.modes import ALPHA, SPF
from isotopes import I135, Xe135, Xe135m, Cs135, U238, Th234, Cm244, Cm246, \
    Cf252, Cf250, Cm248, Es253, Fm254, Fm256, Sb135, Sb136, Sb137, Te136, \
    Te135, Sb138, Sn136, Sn137, He4
    
test_data_dir = Path(__file__).parent / 'test_data'
slow = pytest.mark.slow
needs_batman = pytest.mark.skipif(reason="Can't import batman",
                                condition=DecayGraph is None)
...
def needs_data(*fnames):
    ...
    def _data_taker(f):
        ...
        return pytest.mark.skipif(
                reason=f"Can't locate {[str(v) for v in not_located]}. "
                       f"Use the download script in {str(test_data_dir)}",
...
if __name__ == '__main__':
    frmt = ('%(filename)s - %(lineno)d - %(asctime)s - %(levelname)s: '
            '%(message)s')
    ...
    parser.add_argument('-d', '--debug', action='store_true',
                        help="Flag to enable log debug mode")

    args = parser.parse_args()

    level = logging.DEBUG if args.debug else logging.INFO
    logging.basicConfig(filename=args.log, filemode=args.logmode,
                        format=args.format, level=level)
    spf_data = {}
    if args.spf:
        logging.info("Starting up specific spontaneous fission parsing for "
                     "%s", args.spf)
